...
Respond ONLY with a raw JSON object (no markdown, no code fences):
{
  ...
  "critical_detail": "1-2 sentences MAX covering ALL of the following elements that are present on this garment: (1) NECKLINE & TRIM: describe neckline construction and any decorative trim — if trim is SAME COLOR as garment body (self-colored), write 'self-colored [exact shade] [trim type] — NOT white/cream/contrasting'; if trim IS a contrasting color, name it precisely; if neckline is plain, write 'plain [V/round/scoop/square]-neck, no trim'; never omit neckline; (2) CLOSURES & BOWS/RIBBONS: count every button, bow, tie, snap, frog closure — write 'EXACTLY N [type] at [position]' — NEVER write 'several' or 'multiple'; if a decorative bow or ribbon is present, write 'decorative [color] bow at [position] — do NOT omit'; (3) LOGO/BRAND TEXT/GRAPHIC: if any logo, text label, or graphic print is visible on the garment, write exact text content + color + position — e.g. 'small white sans-serif POLO text on left chest — preserve exact position'; if no logo is visible, do not mention it. Combine all present elements into 1-2 concise sentences. If the garment truly has none of the above (plain neckline, zero buttons, no bow, no logo), write 'plain [neckline], no buttons, no bow, no logo'. Examples: 'self-colored dusty-rose ruffle trim at V-neckline — NOT white; EXACTLY 3 self-colored buttons at center-front placket — do NOT add or remove buttons' / 'white contrast lace collar trim at round neckline; decorative white satin bow at neckline center — do NOT omit bow' / 'V-neck with keyhole opening, EXACTLY 1 self-colored fabric-covered button at center-front; small white POLO logo on left chest — preserve exact position' / 'plain round-neck, no trim, no buttons, no bow, no logo'",
