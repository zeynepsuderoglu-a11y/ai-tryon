...
export default function GlassesUpload() {
  const { glassesUrl, setGlassesUrl } = useStudioStore();
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      setGlassesUrl(result.url);
      toast.success("Gözlük yüklendi!");
    } catch (err: any) {
  ...
  if (preview || glassesUrl) {
    return (
      <div className="relative rounded-2xl overflow-hidden bg-[#f5f5f5] group max-w-xs mx-auto">
        <div className="aspect-square relative">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={preview || glassesUrl!} alt="Gözlük" className="w-full h-full object-contain p-4" />
        </div>
        {uploading && (
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-medium text-[#1a1a1a] mb-1">
        {isDragActive ? "Bırakabilirsiniz!" : "Gözlük fotoğrafını yükle"}
      </p>
