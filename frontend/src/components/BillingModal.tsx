...
export default function BillingModal({ isOpen, onClose, packageId }: BillingModalProps) {
  ...
  const validate = (): string | null => {
    ...
    if (tab === "individual") {
      ...
      if (!form.tax_no?.trim()) return "Vergi / T.C. Kimlik No zorunludur.";
      const taxLen = form.tax_no.trim().length;
      if (taxLen !== 10 && taxLen !== 11) return "Vergi No 10 hane, T.C. Kimlik No 11 hane olmalıdır.";
        ...
        <div className="space-y-3">
          {tab === "individual" ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Vergi / T.C. Kimlik No *" value={form.tax_no || ""} onChange={(v) => set("tax_no", v)} placeholder="Vergi No (10) veya T.C. (11 hane)" maxLength={11} />
